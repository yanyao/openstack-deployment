...
export GET_PIP_URL=${GET_PIP_URL:-"http://repo:8181//downloads/get-pip.py"}
...
if [[ "$APT" != "" ]]; then
    apt-get update && apt-get --force-yes -y install git python-all python-dev curl python2.7-dev build-essential libssl-dev libffi-dev < /dev/null
elif [[ "$YUM" != "" ]]; then
...
#popd
cd /opt
wget http://repo:8181/downloads/ansible_v1.9.3-1.tar.gz
tar xfz ansible_v1.9.3-1.tar.gz
